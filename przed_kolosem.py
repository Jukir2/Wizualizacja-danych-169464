...
# sns.lineplot(x=[1, 2, 3, 4], y=[1, 3, 9, 16], label='linia nr1', color='red', marker='o', linestyle=':')
...
# wykres liniowy z wykorzystaniem danych
...
df = pd.DataFrame(data)
sns.set()
plot = sns.barplot(data=df, x='Kontynent', y='Populacja', ci=None, hue='Kontynent', estimator=np.sum, dodge=False,
                   palette=['red', 'green', 'yellow'])
plot.legend(title='Populacja na kontynetach')
